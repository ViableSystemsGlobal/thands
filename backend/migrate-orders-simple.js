...
/**
 * Simple Orders and Customers Migration
 * This script focuses on migrating the essential data:
 * - Users (customers)
 * - Customers table  
 * - Orders
 * - Order items
 * 
 * Skips cart and wishlist items as they contain old session data
 */
...
console.log('🚀 Starting Simple Orders and Customers Migration');
...
// Helper function to convert integer ID to UUID format (like our products use)
const intToUUID = (intId) => {
  const paddedId = intId.toString().padStart(12, '0');
  ...
  try {
    ...
    for (const item of orderItems) {
      try {
        ...
        // Convert product_id to UUID format for checking
        const productId = intToUUID(parseInt(item.product_id));
        ...
        const existingItem = await query(
          ...
          [orderMappingEntry.postgresId, productId, item.size || null]
        );
        ...
        if (!DRY_RUN) {
          log(`Converting product_id ${item.product_id} to ${productId}`);
          
          await query(`
            INSERT INTO order_items (
              id, order_id, product_id, quantity, size, price, created_at
...
const runMigration = async () => {
  try {
    ...
    console.log('');
    console.log('ℹ️  Note: Cart and wishlist items were skipped as they contain old session data');
